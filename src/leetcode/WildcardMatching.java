...
/**
 * Date: 2014-08-09
 * Problem -- Wildcard Matching {Others}
...
public class WildcardMatching {
	...
	public static boolean isMatch(String s, String p) {  
        ...
        int afterStar = -1;     // the index of S that matches (right after '*' in P)
        
        // if (i matches j || p.charAt(j) == '?'), i++ & j++.  
        // posOfS, starIndex, record the positon of '*' in s and p, j++ and go on.  
        // if not match, go back to star, ++afterStar  
        while (i < sLength) {  
            if (j < pLength && (s.charAt(i) == p.charAt(j) || p.charAt(j) == '?')) {  // match
                i++;  
                j++;  
            ...
            } else if (starIndex != -1) {   // (i) and (j) don't match but there is a former '*'
            	afterStar++; 
            	i = afterStar;  
                j = starIndex + 1;  	// j will always stay at the next right of '*' until MATCHES !!!
