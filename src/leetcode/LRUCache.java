...
public class LRUCache {
    ...
    public void set(int key, int value) {
        ...
        if (map.containsKey(key)) {
			...
			if (len < capacity)    {
			 ...
			 //   if(end != null) {
			 //       end.next = null;  // Can I remove end ???  You can't removeNode the last Node because you cannot locate it in O(1)
			 //   }
    ...
    public void setHead(DoubleLinkedListNode node) {
		...
		if (end == null) {   // this is for the first node inserted  (ONLY the first time !!!)
			end = node;
		}
	}
