...
public class RecursionAndDP {
	...
	public static int numOfStepDP(int n, int[] arr) {
		...
		arr[n] = numOfStepDP(n - 1, arr) + numOfStepDP(n - 2, arr)
				+ numOfStepDP(n - 3, arr);
		return arr[n];
	}
	...
	public static int makeChangeDP(int amount, int[] denoms, int index,
			int[][] map) {
		if (index >= denoms.length - 1)
			return 1;
	 ...
	 * 9.9
	 */
	public static void placeQueens(int row, Integer[] cols,
	...
	public static void highestBoxs(Box[] boxs, boolean[] isUsed,
			Integer[] result, Box prev, int index) {
