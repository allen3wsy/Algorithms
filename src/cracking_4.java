...
public class cracking_4 {
	...
	public static void main(String[] args) {
		...
		System.out.println("four.four : "
				+ createLevelLinkedList2(head).get(2).size());
	...
	/*
	 * Allen's version insert the value into the tree: LevelOrder insertion
	 */
	...
	/*
	 * remove element from BST: Also using Recursion
	 */
	public static TreeNode<Integer> remove(TreeNode<Integer> node, int t) {
		...
		if (t > node.data) { // remember to use recursion right here
			...
			if (node.left != null && node.right != null) {
				...
				node.right = remove(node.right, node.data); // data already
															// removed
	...
	public static TreeNode<Integer> createMinimalBST(int[] arr, int left,
			int right) {
	...
	/*
	 * Allen's solution: Space complexity is O(N): I used another queue to store
	 * the current elements that have not been processed
	 */
	public static ArrayList<LinkedList<TreeNode<Integer>>> createLevelLinkedList(
		...
		/*
		 * when is queue is not empty, we continue doing this levelOrder
		 * traversal
		 */
		while (!queue.isEmpty()) {
			int size = queue.size();
	 ...
	 * CTCI's solution:
	 */
	public static ArrayList<LinkedList<TreeNode<Integer>>> createLevelLinkedList2(
			TreeNode<Integer> root) {
		ArrayList<LinkedList<TreeNode<Integer>>> result = new ArrayList<LinkedList<TreeNode<Integer>>>();
	...
	/**
	 * 
	 * 4.6: It can be tricky to code perfectly: So we need to sketch out the
	 * psuedocode to outline the different cases
	 */
	...
	public static TreeNode<Integer> inorderSucc(TreeNode<Integer> n) {
		...
		if (n.right != null) {
			...
			/*
			 * go all the way up until . NOTE: n == n.parent.right Because we
			 * have to make sure that n.parent is not null, otherwise
			 * n.parent.right would be an error
			 */
	...
	public static TreeNode<Integer> commonAncestorHelper(
			TreeNode<Integer> root, TreeNode<Integer> n1, TreeNode<Integer> n2) {
	...
	/* find the Sum */
	public static void findSum(TreeNode<Integer> root, int[] path, int sum,
			int level) {
		if (root == null)
			return;
		...
		/* \ */
		int temp = 0;
		for (int i = level; i >= 0; i--) { // counting from this level backwards
			temp += path[i];
			if (temp == sum) {
				print(path, i, level);
			}
	...
	/**
	 * 4.10 Decide whether a tree is symmetric using Recursion
	 */
	public static boolean mirror(TreeNode<Integer> t1, TreeNode<Integer> t2) {
		...
		return ((t1.data == t2.data) && mirror(t1.left, t2.right) && mirror(
				t1.right, t2.left));
