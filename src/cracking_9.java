...
public class cracking_9 {
	...
	public static int numOfPathWithLimit(int x, int y, int x1, int y1) {
		...
		return numOfPathWithLimit(x, y - 1, x1, y1)
				+ numOfPathWithLimit(x - 1, y, x1, y1);
	}
	...
	public static int findMagic(int[] array, int left, int right) {
		...
		/*
		 * search left, if we can find the magic number from the left side, then
		 * we don't have to search right....
		 */
	...
	public static ArrayList<ArrayList<Integer>> permute(int[] num) {
		...
		ArrayList<Integer> tmp = new ArrayList<Integer>(); // temp is a null
															// arrayList
		int n = num.length;
	...
	private static void permuteImp(ArrayList<ArrayList<Integer>> result,
			ArrayList<Integer> tmp, int[] num, boolean[] visited) {
		if (tmp.size() == num.length) { // otherwise the length is incorrect
			result.add(new ArrayList<Integer>(tmp)); // copy a new one and add
														// it into the result
														// set
			return;
		}
		...
		// For example: [1, 2, 3]: look at the notes for the recursion tree and
		// the for loop graph
		for (int i = 0; i < num.length; i++) {
			if (!visited[i]) { // if num[i] has not been visited !
				...
				visited[i] = false;
				tmp.remove(tmp.size() - 1); // remove the last one in the temp
											// arrayList
			}
		}
	}
	 ...
	 * @return
	 */
	public static int makeChangeDP(int amount, int[] coins, int index,
			int[][] map) {

		if (index >= coins.length - 1) // the rightmost column is always 1:
										// because that column you can only use
										// 1 cent!!!
		...
		for (int i = 0; i * coinValue <= amount; i++) {
			...
			ways += makeChangeDP(amountRemaining, coins, index + 1, map); // index+1
																			// here:
																			// RECURSE
																			// here
		}
	...
	/*
	 * Check if (row, column) is a valid spot for a queen by checking if there
	 * is a queen in the same column or diagonal. We don't need to check it for
	 * queens in the same row because the calling place Queen only attempts to
	 * place one queen at a time. We know this row is empty ALREADY.
	 */
	...
	public static void main(String[] args) {
		...
		// and also notice that the order of printed map[amount][index] to see
