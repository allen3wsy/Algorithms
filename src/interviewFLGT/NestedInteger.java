...
class NestedSolution {
	...
	public int depthSum(List<NestedInteger> input) {
		...
		// Implementation here
		int sum = 0;
		for (NestedInteger childNode : input) {
			sum += depthSum(childNode, 1);
		}
		return sum;
	}
	...
	// {{}{1,1}1}
	// {{{{{{{{{{{{{{{{{{{{{{{1}}}}}}}}}}}}}}}}}}}}}}}
	public int depthSum(NestedInteger input, int depth) {

		// base case
		if (input.getInteger() != null) {
			return input.getInteger() * depth;
		}
		// recursion
		int sum = 0;
		
		List<NestedInteger> list = input.getList();
		for (NestedInteger childNode : list) {
			sum += depthSum(childNode, depth + 1);
		}
		return sum;
	}
