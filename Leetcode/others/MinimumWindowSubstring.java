...
public class MinimumWindowSubstring {
    ...
    public static String minWindow(String S, String T) {
         ...
         for (int i = 0; i < S.length(); i++) { // i is the current end point of window
             if (alreadyFound.containsKey(S.charAt(i))) {
                 alreadyFound.put(S.charAt(i), alreadyFound.get(S.charAt(i)) + 1);
                 ...
                 if (len == T.length())   {
                     ...
                     while (!needFind.containsKey(S.charAt(start)) || alreadyFound.get(S.charAt(start)) > needFind.get(S.charAt(start)))   {
                         if (needFind.containsKey(S.charAt(start)))  // the latter condition meets !!!
                             alreadyFound.put(S.charAt(start), alreadyFound.get(S.charAt(start)) - 1);
                         start++;
                     }
         ...
         if (windowStart == -1) // which means "len never reaches T.length" (not found) !!!
             return "";
         return S.substring(windowStart, windowEnd + 1);  // [minStart, minEnd + 1)
    }
